...
**NOTE 2:** although the section below is not in-depth documentation, it does cover a fair degree of geeky knowledge. If you want to avoid uncharted waters (and be a chicken), please proceed to [CSV-SQLite Converter](#csv-sqlite-converter) section. I have warned you - your move chief.

## CSV files
CSV (comma-separated values) files allow to store data in a structured manner. The first raw indicates columns' names. The rest is actual data. It looks more or less like this:
...
Last, but not least, CSV is prevalent in Data Science world. I don't know the specific reason, but most datasets (whatever it is - sentences, protein sequences etc.) are available as CSV files. Since I've been tinkering with [language processing models](https://en.wikipedia.org/wiki/Natural_language_processing) recently, I find it natural to use CSV.
...
CSV files seem ideal for _storing_ data. However, this data needs to be somehow _transferred_ from SQLite database to CSV files. There _has to_ be an additional step, a script, that would make this transfer possible. This is where the concept of package comes into play.
...
# Final remark
It was a long read and tough journey through the cybernetic jungle. At the end, I just want to go over what we've covered in this article and possibly share my thoughts on the process.
